...
推奨がある場合は、ラベルの末尾に「(推奨)」を付けるだけにし、並び順は変えません。
...
**Codex 主導**：Codex app の Plan mode など、構造化された選択 UI を表示できる状態なら、その選択 UI で示します。  
ただし Codex が自発的に Plan mode へ切り替えることはできません。通常チャット、CLI、`codex exec`、非対話実行では、本文に 3 択を明記してユーザの返信を待ちます。  
...
| ファイル | 誰が書くか | 中身 |
|---|---|---|
| `round-<N>-<reviewer>.md` | CLI（`codex` / `claude`）<br>主セッション（`subagent`） | レビュアーの出力全文。`subagent` 経路はサブエージェントの出力を**主セッションがこのファイルへ貼ります** |
...
- `<details>` で折りたたんだレビュー出力

レビュー出力の節は末尾 40,000 文字までで、超えた分は載せず、`<summary>` に切り詰めた旨と全文の在り処（`.cross-review/round-<N>-<reviewer>.md`）を書きます（GitHub のコメント上限 65,536 文字に対して、判断本文と確認内容の余地を残すためです）。  
レビュアーの出力そのものが 4,194,304 文字（UTF-16 コード単位。バイト数ではない）を超えた場合は保存の時点で先頭を捨てて末尾だけを残すので、保存ファイルの先頭にその注記が入り、`<summary>` にも「全文ではない」旨が出ます。  
