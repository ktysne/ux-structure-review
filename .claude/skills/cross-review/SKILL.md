...
Codex 主導では Plan mode 等の選択 UI があればそれを使い、無ければ本文に 3 択を明記して返信を待つ。
並び順は固定で、推奨はラベル末尾に「(推奨)」を付けるだけにし、並び順は変えない。
...
- PR コメントは手で組み立てず、裏取りと対応を `.cross-review/round-<N>-triage.md` に書いてから `node tools/cross-review.js comment --round <N> [--verify <検証出力>]` で本文を生成し、`gh pr comment <番号> --body-file <生成物>` で投稿する (レビュー出力は往復ごとに `.cross-review/` へ自動保存される。`subagent` 経路はサブエージェントの出力を `round-<N>-subagent.md` へ貼ってから使う。詳細は `docs/cross-review.md`)。
